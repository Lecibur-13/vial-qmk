...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        // Detecta si Layer 1 está activa (Fn held)
        if (layer_state & (1UL << 1)) {
            switch (keycode) {
                case KC_QUOTE:  // Tecla ' en layer base
                    unregister_mods(MOD_MASK_SHIFT);  // Limpia shift temporal
                    if (get_mods() | get_oneshot_mods()) {
                        send_unicode_string("Ñ");  // Con shift o caps
                    } else {
                        send_unicode_string("ñ");  // Sin shift
                    }
                    set_mods(MOD_MASK_SHIFT);  // Restaura shift si había
                    return false;
            }
        }
    }
